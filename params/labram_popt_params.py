#!/usr/bin/env python3
"""
Created on 16:26, Jul. 21st, 2024
...
__all__ = [
    "labram_popt_params",
    "labram_popt_vqvae_params",
    "labram_popt_mae_params",
    "labram_popt_cls_params",
]

# def labram_popt_params class
class labram_popt_params(DotDict):
    """
    ...
    def __init__(self, dataset="eeg_zhou2023cibr"):
        ...
        super(labram_popt_params, self).__init__()

        ## Generate all parameters hierarchically.
        # -- Model parameters
        self.model = labram_popt_params._gen_model_params(dataset)
        # -- Train parameters
        self.train = labram_popt_params._gen_train_params(dataset)

        ## Do init iteration.
        labram_popt_params.iteration(self, 0)
    ...
    def _gen_model_params(dataset):
        ...
        # Precision parameter.
        model_params.precision = getattr(torch, labram_popt_params._precision)\
            if hasattr(torch, labram_popt_params._precision) else torch.float32
        ...
        model_params.tokenizer = labram_popt_params._gen_model_tokenizer_params(model_params)
        ## -- Encoder parameters
        model_params.encoder = labram_popt_params._gen_model_encoder_params(model_params)
        ## -- Vector-Quantizer parameters
        model_params.vq = labram_popt_params._gen_model_vq_params(model_params)
    ...
    def _gen_model_encoder_params(model_params):
        ...
        ## -- Normal parameters
        # The number of coordinate axes.
        model_encoder_params.n_axes = model_params.n_axes
    ...
    def _gen_train_params(dataset):
        ...
        # Precision parameter.
        train_params.precision = getattr(torch, labram_popt_params._precision)\
            if hasattr(torch, labram_popt_params._precision) else torch.float32
...
# def labram_popt_vqvae_params class
class labram_popt_vqvae_params(labram_popt_params):
    """
    ...
    def __init__(self, dataset="eeg_zhou2023cibr"):
        """
        Initialize `labram_popt_vqvae_params` object.
        """
        ## First call super class init function to set up `DotDict`
        ## style object and inherit it's functionality.
        super(labram_popt_vqvae_params, self).__init__(dataset=dataset)
        ...
        ## Do init iteration.
        labram_popt_vqvae_params.iteration(self, 0)
    ...
    def iteration(self, iteration):
        ...
        super(labram_popt_vqvae_params, self).iteration(iteration)
        ## -- Train parameters
        # Calculate current learning rate.
        lr_min, lr_max = self.train.lr_factors
        # If `iteration` is smaller than `params.train.warmup_epochs`, gradually increase `lr`.
...
# def labram_popt_mae_params class
class labram_popt_mae_params(labram_popt_params):
    """
    This contains one single object that generates a dictionary of parameters,
    ...
    def __init__(self, dataset="eeg_zhou2023cibr"):
        """
        Initialize `labram_popt_mae_params` object.
        """
        ## First call super class init function to set up `DotDict`
        ## style object and inherit it's functionality.
        super(labram_popt_mae_params, self).__init__(dataset=dataset)
        ...
        ## Do init iteration.
        labram_popt_mae_params.iteration(self, 0)
    ...
    def iteration(self, iteration):
        ...
        """
        ## Iterate super parameters.
        super(labram_popt_mae_params, self).iteration(iteration)
        ## -- Train parameters
        # Calculate current learning rate.
        lr_min, lr_max = self.train.lr_factors
        # If `iteration` is smaller than `params.train.warmup_epochs`, gradually increase `lr`.
...
# def labram_popt_cls_params class
class labram_popt_cls_params(labram_popt_params):
    """
    This contains one single object that generates a dictionary of parameters,
    ...
    def __init__(self, dataset="eeg_zhou2023cibr"):
        """
        Initialize `labram_popt_cls_params` object.
        """
        ## First call super class init function to set up `DotDict`
        ## style object and inherit it's functionality.
        super(labram_popt_cls_params, self).__init__(dataset=dataset)
        ...
        ## Do init iteration.
        labram_popt_cls_params.iteration(self, 0)
    ...
    def iteration(self, iteration):
        ...
        """
        ## Iterate super parameters.
        super(labram_popt_cls_params, self).iteration(iteration)
        ## -- Train parameters
        # Calculate current learning rate.
        lr_min, lr_max = self.train.lr_factors
        # If `iteration` is smaller than `params.train.warmup_epochs`, gradually increase `lr`.
...
if __name__ == "__main__":
    # Instantiate `labram_popt_params`.
    labram_popt_params_inst = labram_popt_params(dataset="eeg_zhou2023cibr")
    # Instantiate `labram_popt_vqvae_params`.
    labram_popt_vqvae_params_inst = labram_popt_vqvae_params(dataset="eeg_zhou2023cibr")
    # Instantiate `labram_popt_mae_params`.
    labram_popt_mae_params_inst = labram_popt_mae_params(dataset="eeg_zhou2023cibr")
    # Instantiate `labram_popt_cls_params`.
    labram_popt_cls_params_inst = labram_popt_cls_params(dataset="eeg_zhou2023cibr")
